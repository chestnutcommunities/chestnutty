...
if (have_posts()):
    ...
    if (is_search()) {
        get_ad('468x60');
        echo '<h1>';
        printf(__('Search Results for: %s', 'chestnut-adsense' ), '<span class="search-keyword">'.get_search_query().'</span>');
        echo '</h1>';
    }
    ...
    if (is_tag()) {
        get_ad('468x60');
        echo '<h1>';
        printf(__('Tag Archives: %s', 'chestnut-adsense' ), '<span class="tag">'.single_tag_title('', false).'</span>');
        echo '</h1>';
    }
...
?>
    <div class="post">
        ...
        <div class="post-content no-content">
            <p>
                <?php _e('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'chestnut-adsense'); ?>
            </p>
        </div>
    </div>
<?php
